from api2108 import Common
from api2108.Sendhttp import SendHttp
import unittest
import json
import requests
...
class Action(unittest.TestCase):
    ...
    def del_address(self):
        ...
        if(count.__len__() > 1):
            para = {"id": count[0]["id"]}
            res = SendHttp().sent_post_bycookies(del_url, para, self.user_login())
            # print("收货地址已删除")
        else:
            print("收货地址为空")
    ...
    def default_para(self):
        ...
        res = SendHttp().sent_post_bycookies(url, para, self.user_login())
        return res
