...
public class HttpDriver {
	...
	public	static String doGet(String url) throws Exception {
		get = new HttpGet(ReadPro.getProValue("BaseUrl")+url);
	...
	public static String doGet(String url, JSONObject data) throws Exception {
		...
		httpClient = HttpClients.createDefault();
		HttpGet get = new HttpGet(ReadPro.getProValue("BaseUrl")+url + "?" + para);
		get.addHeader("Content-Type", "application/json");
		respone = httpClient.execute(get);
		HttpEntity entity = respone.getEntity();
		content = EntityUtils.toString(entity, "utf-8");
	...
	public static String doGet(String url, Map<String, Object> para) {
		...
		HttpGet get = new HttpGet(ReadPro.getProValue("BaseUrl")+url + "?" + mapToString(para));
		get.addHeader("Content-Type", "application/json");
		try {
			respone = httpClient.execute(get);
		} catch (IOException e) {
	...
	public static String doGet(String url, CookieStore cookie) throws Exception{
		...
		HttpGet get = new HttpGet(ReadPro.getProValue("BaseUrl")+url);
	...
	public static String doGet(String url, JSONObject para, CookieStore cookie) throws Exception {
		String data = URLEncoder.encode(para.toString(), "UTF-8");
		RequestConfig gConfig = RequestConfig.custom().setCookieSpec(CookieSpecs.STANDARD).build();
		httpClient = HttpClients.custom().setDefaultRequestConfig(gConfig).setDefaultCookieStore(cookie).build();	
		HttpGet get = new HttpGet(ReadPro.getProValue("BaseUrl")+url + "?" + data);
		System.out.println(get);
		get.addHeader("Content-Type", "application/json");
		respone = httpClient.execute(get);
		HttpEntity entity = respone.getEntity();
	...
	public static String doGet(String url, Map<String,Object> para, CookieStore cookie) throws Exception, IOException {
		RequestConfig gConfig = RequestConfig.custom().setCookieSpec(CookieSpecs.STANDARD).build();
		httpClient = HttpClients.custom().setDefaultRequestConfig(gConfig).setDefaultCookieStore(cookie).build();	
		HttpGet get = new HttpGet(ReadPro.getProValue("BaseUrl")+url + "?" + mapToString(para));
		get.addHeader("Content-Type", "application/json");
